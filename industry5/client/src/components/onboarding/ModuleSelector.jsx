...
import { Factory, Cloud, Truck, ShoppingBag, ArrowRight, Settings } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl w-full">
        {/* Standard Preset Industries */}
        {Object.values(INDUSTRY_PRESETS)
          .filter(industry => ['manufacturing', 'logistics', 'saas'].includes(industry.id))
          .map((industry) => {
        ...
        {/* Custom Industry - On Hold */}
        <div className="group bg-slate-900/50 border border-slate-800 p-6 rounded-2xl text-left flex flex-col h-full relative overflow-hidden opacity-60 cursor-not-allowed">
          <div className="w-12 h-12 bg-slate-800 rounded-xl flex items-center justify-center text-slate-500 mb-6">
            <Settings size={24} />
          </div>

          <h3 className="text-xl font-bold text-slate-300 mb-2">Custom Industry</h3>
          <p className="text-slate-500 text-sm mb-6 flex-1">
            Create a fully custom logic model for niche industries.
          </p>

          <div className="flex items-center text-slate-500 font-medium text-xs uppercase tracking-wider border border-slate-700 w-fit px-2 py-1 rounded">
            Coming Soon
          </div>
        </div>
      </div>
